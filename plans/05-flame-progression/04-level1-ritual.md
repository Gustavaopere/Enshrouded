...
- `FlameRitualExecutor` performs eligibility, consumption/result application and progression mutation transactionally/idempotently enough that reconnect/double invocation cannot grant the same ritual twice.
...
**Acceptance:** Stage 05 provides a complete, provider-agnostic ritual execution/checkpoint engine before the altar adapter exists; Stage 06 can later bind the authentic first Lich Skull without any causal dependency cycle.
