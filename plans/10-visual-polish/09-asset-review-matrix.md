...
|---|---|---|---|---|---|
| Flame Altar 3×3 lifecycle | `FlameAltarBlockEntity`; `FlameAltarStructureValidator`; `FlameAltarStructureLifecycle`; `FlameAltarChunkRecoveryEvents`; `FlameAltarRestartGameTests` | existing Flame Altar controller + canonical Flame Ward/Sanctuary runtime | explicit formation; exact center + four inward-facing braces + four corner runes; bounded loaded-only validation; deterministic unform/reform; fail-closed restart revalidation | gameplay automation complete; visual capture still pending | **REVIEW_IN_GAME** |
