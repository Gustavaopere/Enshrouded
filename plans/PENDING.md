...
## Foundation-owned ward boundary — Foundation implementation still required

- `ENSH-L1-FLAME-WARD-001`: Foundation owns `FlameWardQuery` per `DECISIONS.md` decision 33 so Stage 01 query, Stage 02 terrain safety and Stage 03 exposure do not depend on Stage 05 implementation classes. The Foundation fallback must report no suppression. `ShroudSample` must retain canonical logical intensity/severity/source and use `sanctuarySuppressed` as the effective overlay; exposure treats suppression as safe and `MutationAuthority` vetoes terrain using the same ward query. Stage 05 `FlameWardService` later implements the interface through an indexed altar/ward service. Implement this Foundation contract test-first before accepting Stage 00, then keep the pending contract open until Stage 01/02/03 consume it and Stage 05 proves the real ward service without logical-field rewriting.
