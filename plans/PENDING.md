...
- `ENSH-CI-ACTIONS-001`: Foundation acceptance is blocked by GitHub Actions jobs terminating before runner initialization. Enshrouded push and pull-request runs create a `verify` job but expose no steps; on current Foundation HEAD `67e5cb4af0f29d9e14f5b21d8025cdb62e6d8331`, workflow `33119758860` / job `98683408591` reports `steps=[]`, `runner_id=0`, `runner_name=""` and completes in roughly three seconds. This proves the current failure occurs before GitHub allocates a runner, so it is not evidence of a failing checkout, Gradle command, unit test, build, GameTest or server harness. Reruns reproduce the same result. A private cross-repository control on `Gustavaopere/Volcanoes` (`33099719939` / job `98615923587`) showed the same pre-step failure pattern during the same period, so the blocker is not specific to Enshrouded's workflow or repository. The repository APIs available here do not expose account billing/quota state, so no specific billing cause is asserted. The conversation-local fallback also cannot perform a real NeoForge build because `services.gradle.org` is not resolvable and no suitable Gradle/NeoForge cache is present. Do not mark Foundation complete, rename tasks with `✅-`, merge PR #2, or create Stage 01 branches until a real runner executes wrapper integrity, unit tests, diff sanity, NeoForge build/JAR sanity, GameTests and the two-boot dedicated-server save/reload harness GREEN.
