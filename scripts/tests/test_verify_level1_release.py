...
class Level1ReleaseContractTest(unittest.TestCase):
    def make_repo(self):
        ...
        (root / "docs/release/LEVEL-1-RELEASE-CHECKLIST.md").write_text(
            "NeoForge 1.21.1\nJava 21\nMANUAL_CURRENT_PACK_SMOKE_REQUIRED\nNo unresolved P0/P1 release blockers\n",
            encoding="utf-8",
        )
        (root / "docs/release/LEVEL-1-RELEASE-NOTES.md").write_text("Level 1\nNeoForge 1.21.1\nJava 21\n", encoding="utf-8")
        ...
        (root / "src/main/resources/META-INF/neoforge.mods.toml").write_text('modLoader="javafml"\n', encoding="utf-8")
        ...
        (root / "src/main/resources/assets/enshrouded/lang/pt_br.json").write_text(json.dumps({**lang, "hud.enshrouded.shroud": "Mortalha"}), encoding="utf-8")
        return root
    ...
    def test_missing_release_doc_fails_closed(self):
        root = self.make_repo(); (root / "docs/release/LEVEL-1-RELEASE-CHECKLIST.md").unlink()
    ...
    def test_missing_prerequisite_closeout_fails_closed(self):
        root = self.make_repo(); (root / "plans/09-hardening/✅-05-third-party-licenses-provenance.md").unlink()
        self.assertTrue(any("09.05" in e for e in release.validate_repository(root)))

    def test_current_pack_versions_are_required(self):
        root = self.make_repo(); p = root / "docs/compat/current-pack-2026-09-06.md"
    ...
    def test_language_key_parity_is_required(self):
        root = self.make_repo(); p = root / "src/main/resources/assets/enshrouded/lang/pt_br.json"
        data = json.loads(p.read_text(encoding="utf-8")); data.pop("hud.enshrouded.shroud"); p.write_text(json.dumps(data), encoding="utf-8")
    ...
    def test_unresolved_release_blocker_fails_closed(self):
        root = self.make_repo(); (root / "docs/release/LEVEL-1-RELEASE-NOTES.md").write_text("REVIEW_REQUIRED\n", encoding="utf-8")
    ...
    def test_manual_full_pack_smoke_must_be_explicit(self):
        root = self.make_repo(); p = root / "docs/release/LEVEL-1-RELEASE-CHECKLIST.md"
